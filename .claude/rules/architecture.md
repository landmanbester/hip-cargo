...
* **Mount Resolution:** Volume mounts are resolved automatically from Path-like type hints (ro) and `@stimela_output` decorators (rw). Inputs with `writable: true` (e.g. an MS modified in place) are also mounted rw. Backend priority is: `apptainer` → `singularity` → `docker` → `podman`.
